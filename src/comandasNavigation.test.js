...
import { workspaceHarness, buttonNamed, nodeText, renderWithNavigation } from './test-support/renderWorkspace.js'
...
for (const component of ['Sidebar', 'MobileNavigation']) {
  test(`${component} opens Comandas directly after Pedidos and marks it current`, async (t) => {
    ...
    const { default: Navigation } = await harness.load(`/src/app/shell/${component}.jsx`)
    const destinations = []
    const renderer = await renderWithNavigation(harness, Navigation, { activeTab: 'comandas', granted, implemented, onNavigate: (id) => destinations.push(id) })

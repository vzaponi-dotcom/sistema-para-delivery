...
const inferLegacyOrderDate = (order, now) => {
  ...
  if (order.date === 'Ontem') { const yesterday = new Date(now); yesterday.setDate(yesterday.getDate() - 1); return toLocalDateValue(yesterday) }
...
const inferLegacyTimestamp = (order, now) => {
  ...
  if (order.date === 'Ontem') reference.setDate(reference.getDate() - 1)
...
export const normalizeOrder = (order, now = new Date()) => {
  ...
  return { ...order, status: finishedAt ? 'Finalizado' : 'Em preparo', orderDate, date: formatOrderDate(orderDate), createdAt, finishedAt }
}

export const isOrderFinished = (order) => Boolean(parseDate(order?.finishedAt))
export const getElapsedMinutes = (order, now = new Date()) => { const createdAt = parseDate(order?.createdAt); const reference = new Date(now); if (!createdAt || Number.isNaN(reference.getTime())) return 0; return Math.max(0, Math.floor((reference.getTime() - createdAt.getTime()) / 60_000)) }
export const getOrderTimingState = (order, now = new Date()) => { const elapsedMinutes = getElapsedMinutes(order, now); if (elapsedMinutes > 40) return 'very-late'; if (elapsedMinutes > 30) return 'late'; return 'on-time' }

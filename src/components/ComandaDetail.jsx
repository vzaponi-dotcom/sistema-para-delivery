...
function ComandaDetail({ detail, currency, disabled = false, busyAction = false, printingDisabled = false, canTransfer = false, canCreateOrders = true, canExecutePrinting = true, onAddOrder, onViewTicket, onPrint, onPay, onTransfer, labelledBy }) {
  ...
  return (
    <section className="comanda-detail" aria-labelledby={labelledBy} aria-label={labelledBy ? undefined : `Comanda ${detail.number}`}>
      ...
      <div className="comanda-detail-actions">
        {canCreateOrders && <Button type="button" disabled={blocked} onClick={() => { if (canCreateOrders && !blocked) onAddOrder?.() }}>Adicionar pedido</Button>}
        {canTransfer && <Button type="button" variant="secondary" disabled={blocked} onClick={() => { if (!blocked) onTransfer?.() }}>Transferir comanda</Button>}
        ...
        <Button type="button" variant="secondary" disabled={blocked || printingDisabled || !canExecutePrinting} onClick={() => { if (canExecutePrinting && !blocked && !printingDisabled) onPrint?.() }}>Imprimir comanda</Button>

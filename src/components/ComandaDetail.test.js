...
test('consolidated detail renders official payable total, quantities, unit prices, variations and counts', async (t) => {
  ...
  const actions = []
  const r = await h.render(Detail, { detail: comandaDetail, currency: (v) => `R$ ${v.toFixed(2)}`, onAddOrder: () => actions.push('add'), onPay: () => actions.push('pay') })
  ...
  assert.equal(r.root.findAllByType('button').length, 2)
  await act(async () => { buttonNamed(r.root, 'Adicionar pedido').props.onClick(); buttonNamed(r.root, 'Registrar pagamento').props.onClick() })
  assert.deepEqual(actions, ['add', 'pay'])
})

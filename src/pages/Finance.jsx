...
function Finance({
  ...
  return (
    <>
      ...
      <div className="finance-opening-balance-actions">
        <Button type="button" variant="secondary" onClick={onConfigureOpeningBalance} disabled={writeDisabled}>
          {financeSettings ? 'Editar saldo inicial' : 'Configurar saldo inicial'}
        </Button>
        {financeSettings && (
          <span>
            Abertura em {financeSettings.openingDate} · saldo atual {currentBalance === null || currentBalance === undefined ? 'não disponível' : currency(currentBalance)}
          </span>
        )}
      </div>
            ...
            <article className="movement-row" key={movement.id}>
              ...
              {movement.source === 'manual' && (
                <div className="movement-actions">
                  <Button type="button" variant="secondary" onClick={() => onEditMovement?.(movement)} disabled={writeDisabled}>Editar</Button>

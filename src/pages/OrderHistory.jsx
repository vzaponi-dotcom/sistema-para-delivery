...
function OrderHistory({ orders, currency, onCancelOrder, actionKey = null }) {
      ...
      <section className="surface-card order-history-surface">
        ...
        <div className="order-history-list">
              ...
              <article className="order-history-row" key={order.id}>
                ...
                <div className="order-history-main">
                  ...
                  {order.status === 'Cancelado' && (
                    <small className="order-cancel-meta">Motivo: {cancelReason}{order.cancelReasonNote ? ` · ${order.cancelReasonNote}` : ''}</small>
                  )}
                </div>
                ...
                <div className="order-history-actions">
                  ...
                  {order.status === 'Finalizado' && (
                    <Button type="button" variant="secondary" className="button-danger-outline" disabled={Boolean(actionKey)} onClick={() => onCancelOrder(order)}>Cancelar pedido</Button>
                  )}
                </div>
              </article>
            )
          ...
          {!terminalOrders.length && (
            <div className="empty-state compact-empty-state"><Icon name="orders" size={28} /><strong>Nenhum pedido neste filtro</strong><span>Os pedidos finalizados e cancelados aparecerão aqui.</span></div>
          )}
        </div>
      </section>

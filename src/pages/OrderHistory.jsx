...
function OrderHistory({ orders = [], currency = defaultCurrency, onCancelOrder, actionKey = null }) {
  ...
  const confirmCancellation = async (payload) => {
    ...
    setSubmitting(true)
    try {
      const saved = await onCancelOrder(cancelOrder.id, payload)
            ...
            return (
              <article className="order-history-row" key={order.id}>
                ...
                <div className="order-history-actions">
                  ...
                  {order.status === 'Finalizado' && <Button type="button" variant="secondary" className="button-danger-outline" disabled={Boolean(actionKey) || submitting} onClick={() => setCancelOrder(order)}>Cancelar pedido</Button>}
                </div>
              </article>
            )
          })}

...
test('client rows and product form remain touchable at 320px', async () => {
  ...
  assert.match(clients, /min-height:\s*(?:44|48|58)px/)
...
test('product list moves actions out of the compressed name column on narrow screens', async () => {
  ...
  assert.match(appCss, /@media\s*\(max-width:\s*640px\)[\s\S]*\.product-row\s*\{[^}]*grid-template-columns:\s*38px\s+minmax\(0,\s*1fr\)/s)
  assert.match(appCss, /@media\s*\(max-width:\s*640px\)[\s\S]*\.product-row \.entity-actions\s*\{[^}]*grid-column:\s*1\s*\/\s*-1[^}]*flex-direction:\s*row/s)
  assert.match(appCss, /@media\s*\(max-width:\s*640px\)[\s\S]*\.product-row \.icon-button\s*\{[^}]*width:\s*44px[^}]*height:\s*44px/s)
  assert.match(appCss, /@media\s*\(max-width:\s*640px\)[\s\S]*\.product-main strong\s*\{[^}]*white-space:\s*normal[^}]*overflow-wrap:\s*anywhere/s)
})

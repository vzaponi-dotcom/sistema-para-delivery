...
test('destino financeiro explícito negado não faz fallback', async (t) => {
  ...
  await h.renderAdminApp(Probe, { ref: api })
...
test('uma única subnavegação precede o PageHeader nas áreas que mantêm subtabs', async (t) => {
  const h = await workspaceHarness(t)
  const [{ default: Orders }, { default: OrderHistory }, { default: DashboardSurface }, { default: Receivables }, { default: Finance }] = await Promise.all([
    h.load('/src/domains/orders/ui/Orders.jsx'), h.load('/src/domains/orders/ui/OrderHistory.jsx'), h.load('/src/app/surfaces/dashboard/DashboardSurface.jsx'), h.load('/src/domains/finance/ui/Receivables.jsx'), h.load('/src/domains/finance/ui/Finance.jsx'),
  ])
...
test('Cadastros permanece sem AreaNavigation', async (t) => {
  ...
  const [{ default: Clients }, { default: Products }, { Tables }] = await Promise.all([
    h.load('/src/domains/customers/ui/Clients.jsx'), h.load('/src/domains/catalog/ui/Products.jsx'), h.load('/src/domains/table-service/index.js'),
  ])
...
test('controlador fecha Mais antes da confirmação, cancela preservando e confirma uma vez', async (t) => {
  ...
  const { renderer, updateApp } = await h.renderAdminApp(Probe, { ref: api }, { initialEntries: ['/clientes'] })
  ...
  dirty = true
  await updateApp({ ref: api, checkoutPending: true })
  await act(async () => api.current.completeNavigation('new-order'))
  await act(async () => api.current.requestNavigation('clients'))
...
test('destino revogado antes da confirmação não descarta o rascunho', async (t) => {
  ...
  const { renderer, updateApp } = await h.renderAdminApp(Probe, { ref: api, caps: new Set(['orders.create', 'clients.view']) }, { initialEntries: ['/clientes'] })
  await act(async () => api.current.completeNavigation('new-order'))
  await act(async () => api.current.requestNavigation('clients'))
  await updateApp({ ref: api, caps: new Set(['orders.create']) })
  await act(async () => api.current.confirmDiscard())
  assert.equal(discards, 0)
  assert.equal(api.current.pendingDestination, null)
})

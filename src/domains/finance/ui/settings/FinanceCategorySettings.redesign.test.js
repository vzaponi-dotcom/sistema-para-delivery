...
test('finance categories follow the approved grouped table contract with icons and sortable rows', async (t) => {
  ...
  assert.match(nodeText(row(screen.root, 'contribution')), /1.*Aporte.*Receita/s)
  assert.match(nodeText(row(screen.root, 'supplies')), /1.*Insumos.*Despesa/s)
  assert.doesNotMatch(nodeText(row(screen.root, 'contribution')), /Ativa|Inativa/)
  assert.doesNotMatch(nodeText(row(screen.root, 'supplies')), /Ativa|Inativa/)
...
test('mobile finance categories render compact cards with horizontal type and status metadata', async (t) => {
  ...
  assert.match(nodeText(metadata), /Receita/s)
  assert.doesNotMatch(nodeText(metadata), /Ativa|Inativa/)

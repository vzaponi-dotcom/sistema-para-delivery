...
test('overview presents official KPIs and preserves receivables as navigation only', async () => {
  ...
  const receivableLink = await readFile(new URL('./ReportingReceivableLink.jsx', import.meta.url), 'utf8')
  ...
  assert.match(source, /ReportingReceivableLink/)
  assert.match(receivableLink, /Gerenciar em A receber/)
  assert.match(receivableLink, /\/financeiro\/a-receber/)

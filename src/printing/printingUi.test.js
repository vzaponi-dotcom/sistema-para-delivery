...
test('physical popup remains separate from remote queue decisions', async () => {
  ...
  assert.match(overlayHook, /printing\.printSecondCopy\(secondCopyPromptJob\)/)
  assert.match(overlays, /'Parar por agora' : 'Depois'/)
  assert.match(queue, /printing\?\.requestSecondCopy\?\.\(selectedJob, \{ refreshManager: false \}\)/)
  assert.match(queue, /printing\?\.skipSecondCopy\?\.\(selectedJob, \{ refreshManager: false \}\)/)
...
test('successful active order creation confirms queueing without invoking a local transport', () => {
  ...
  const end = app.indexOf('function showApiError', start)

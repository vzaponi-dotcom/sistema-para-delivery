...
export async function workspaceHarness(t, { mobile = false, userAgent = 'test' } = {}) {
  ...
  for (const [key, value] of Object.entries({ window, document, localStorage, navigator: { onLine: true, userAgent }, addEventListener: window.addEventListener.bind(window), removeEventListener: window.removeEventListener.bind(window) })) {

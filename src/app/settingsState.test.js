...
test('loaded, edited and discarded states keep confirmed base and draft separate', () => {
  ...
  assert.deepEqual(state.confirmed, adminFixture)
  state = settingsReducer(state, { type: 'discarded' })
  assert.deepEqual(state.draft, adminFixture.data)
  assert.equal(state.dirty, false)
})

test('submitted intent is immutable while later edits remain a separate draft', () => {
  let state = settingsReducer(createSettingsState(), { type: 'loaded', value: adminFixture })
  state = settingsReducer(state, { type: 'edited', data: draftFixture })
  state = settingsReducer(state, { type: 'saveStarted', mutationId: 'save-1', payloadHash: 'hash-1', startedAt: '2026-09-13T10:00:00.000Z' })
  const submitted = structuredClone(state.submitted)
  const laterDraft = { ...draftFixture, defaultModality: 'Retirada' }
  state = settingsReducer(state, { type: 'edited', data: laterDraft })
  ...
  state = settingsReducer(state, { type: 'saveConfirmed', value: saved })
...
test('confirmed save replaces official state while revision conflict preserves all edit inputs', () => {
  ...
  state = settingsReducer(state, { type: 'edited', data: draftFixture })
  state = settingsReducer(state, { type: 'saveStarted', mutationId: 'save-1', payloadHash: 'hash-1', startedAt: '2026-09-13T10:00:00.000Z' })
  const conflicted = settingsReducer(state, { type: 'saveConflict', error: { code: 'SETTINGS_REVISION_CONFLICT' } })
  ...
  const confirmed = settingsReducer(state, { type: 'saveConfirmed', value: saved })

...
const renderMenu = async (t, { granted = new Set(['operations.settings.view', 'preferences.local']), onLogout = () => {}, logoutDisabled = false } = {}) => {
  ...
  const renderer = await h.render(NavigationProvider, {
    ...
    children: React.createElement(OperationMenu, { onLogout, logoutDisabled }),
...
test('operation shortcuts respect official navigation and close after navigation', async (t) => {
  ...
  await act(async () => buttonNamed(renderer.root, 'Amor & Sabor, operação atual').props.onClick())
...
test('About identifies the operation and release without a personal profile', async (t) => {
  ...
  assert.match(copy, /Operação atual: Amor & Sabor/)

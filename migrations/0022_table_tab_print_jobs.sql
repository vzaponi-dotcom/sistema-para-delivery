...
DROP INDEX IF EXISTS print_job_attempts_business_idx;
DROP INDEX IF EXISTS print_job_attempts_station_idx;
...
CREATE TABLE print_jobs_next (
  ...
  CHECK (
    ...
    OR (type = 'table-tab' AND order_id IS NULL AND table_tab_id IS NOT NULL AND trigger = 'manual' AND copies_requested = 1)
...
SELECT
  id, business_id, order_id, NULL, type, trigger, status, priority, parent_job_id,
  copies_requested, copies_printed, station_id, snapshot_json, created_at,
  available_at, processing_started_at, processed_at, discarded_at,
  attention_reason, action_actor_label, action_at, last_error_code,
  last_error_message, second_copy_prompted_at, second_copy_requested_at,
...
CREATE TABLE print_job_attempts_next (
  id TEXT PRIMARY KEY,
  business_id TEXT NOT NULL REFERENCES businesses(id) ON DELETE CASCADE,
  job_id TEXT NOT NULL REFERENCES print_jobs_next(id) ON DELETE CASCADE,
...
INSERT INTO print_job_attempts_next SELECT * FROM print_job_attempts;

DROP TABLE print_job_attempts;
DROP TABLE print_jobs;
ALTER TABLE print_jobs_next RENAME TO print_jobs;
ALTER TABLE print_job_attempts_next RENAME TO print_job_attempts;

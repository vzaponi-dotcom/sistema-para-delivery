...
export const parsePromisedPaymentDate = (value, today) => {
  ...
  if (!isValidCalendarDate(normalized)) throw repositoryError(400, 'INVALID_PROMISED_PAYMENT_DATE', 'Data prometida invÃ¡lida.')
  if (normalized < today) throw repositoryError(400, 'PROMISED_PAYMENT_DATE_IN_PAST', 'A data prometida nÃ£o pode estar no passado.')
  return normalized
}
...
export const updateOrderPaymentPromise = async (db, businessId, orderId, rawDate, now = new Date()) => {
  ...
  if (!order) throw repositoryError(404, 'ORDER_NOT_FOUND', 'Pedido nÃ£o encontrado.')
  if (order.status === 'Cancelado') throw repositoryError(409, 'ORDER_CANCELLED', 'Pedido cancelado nÃ£o pode receber uma promessa de pagamento.')
  if (order.paymentStatus === 'Pago') throw repositoryError(409, 'ORDER_ALREADY_PAID', 'Pedido quitado nÃ£o pode alterar a data prometida.')

...
import { moneyToCents, optionalText, requireNonEmpty, validateIsoDate, validateMovementType, validateOrderType, validatePaymentMethod, validatePositiveInteger } from './validation.js'
...
const orderInput = (body, idempotencyKey) => ({ clientId: requireNonEmpty(body.clientId, 'clientId'), productId: requireNonEmpty(body.productId, 'productId'), type: validateOrderType(body.type), quantity: validatePositiveInteger(body.quantity, 'quantity'), orderDate: validateIsoDate(body.orderDate, 'orderDate'), idempotencyKey: requireNonEmpty(idempotencyKey, 'idempotency-key') })
...
const authenticatedApi = async (request, env) => {
  ...
  if (url.pathname === '/api/orders' && request.method === 'POST') { assertSameOriginMutation(request); const order = await createOrder(env.DB, session.businessId, orderInput(await readJson(request), request.headers.get('idempotency-key'))); return json({ order }, { status: 201 }) }
